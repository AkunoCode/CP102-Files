...
def connect():
    try:
        con = mysql.connector.connect(user='root', password='root', host='localhost', database='Sale')
...
def add(con):
    try:
        while True:
            ...
            # Execute SQL
            cursor = con.cursor()
            sql = "Insert Into tblProduct (Code, Name, Price) Values ('%s', '%s', '%f')" % (code, name, price)
